...
import { cleanup, render, screen, getByText, queryByAttribute, waitFor, fireEvent } from "@testing-library/react";
  ...
  jest.spyOn(global, 'fetch')
    ...
    .mockImplementationOnce(() => Promise.resolve({
      ...
      json: () => Promise.resolve([
        {
          "user_id": 656,
          "submitted_date": "2020-12-19T05:00:00.000Z",
          "rating": 5,
          "review": "Hello world"
        },
        {
          "user_id": 88888,
          "submitted_date": "2021-03-11T05:00:00.000Z",
          "rating": 5,
          "review": "Testing result"
        },
        {
          "user_id": 88888,
          "submitted_date": "2021-03-15T04:00:00.000Z",
          "rating": 5,
          "review": "asdsdasdasadadssad"
        }
      ])
    }))
    .mockImplementationOnce(() => Promise.resolve({
...
describe("Homepage scenario", () => {
  ...
  test('Should render all data', async () => {
    ...
    expect(container.querySelectorAll('div.MuiCardContent-root')).toHaveLength(3);

  });


  test('Click the favorites and shopping list', async () => {

    global.alert = jest.fn();

    fireEvent.click(getByText(container, 'Favourites'));
    expect(global.alert).toHaveBeenCalledWith("Please log in to add favorite!");
    expect(sessionStorage.getItem('logged')).not.toEqual(1);

    window.sessionStorage.setItem('logged', 1);
    fireEvent.click(getByText(container, 'Shopping List'));
    expect(global.alert).toHaveBeenCalledWith("The ingredients have been added to your shopping list!");

    fireEvent.click(getByText(container, 'Favourites'));
    expect(global.alert).toHaveBeenCalledWith("This recipe is added to your favorite!");

  });

  test('Submit comments', async () => {
    global.alert = jest.fn();

    fireEvent.click(getByText(container, 'Submit'));
    expect(global.alert).toHaveBeenCalledWith("Your comment was submitted!");
